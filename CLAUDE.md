...
そのため `npm run check:csp`（`scripts/check-csp-hash.mjs`）が一致を機械的に検査し、CI の `html-validate` ジョブでも走る。**両ページに同じ規則を掛ける**（ページごとに分岐を書かない）:

- **規則 A**: 実効ディレクティブ（`script-src-elem` → `script-src` → `default-src` の優先順）の sha256 トークンの集合と、実行対象インライン script のハッシュの集合が**過不足なく一致する**こと。1 対 1 で突き合わせるので「ハッシュが古い」「script を足したのにトークンを足していない」「script を消したのにトークンが残っている」がすべてこの 1 つで落ちる。**不一致のときは追加・削除すべきトークンをそのまま貼り替えられる形で出す。**
- **規則 B**: インライン script も外部 script も持たないページは `script-src 'none'`（最小権限）。
- **規則 C**: 外部 script があるなら、実効ディレクティブに読み込み元（`'self'` やホスト名）があること。規則 A は sha256 とインライン script の対応しか見ないため、これが無いと `script-src 'none'` のページに `<script src>` を足しても緑で通り、ブラウザは読み込まない。
...
**検査対象のページは一覧を書かず、`git ls-files '*.html'`（追跡中のファイル）から導出する。** 写しを持つと、ページを足した人が追加を忘れた瞬間にそのページだけ黙って検査対象から外れる（規則をページごとに分けなかったのと同じ理由）。ディレクトリを歩く方式ではなく git を使うのは、**GitHub Pages が配信するのはコミットされた内容**だから ——「配信されるページ」と「追跡されているページ」が定義上一致する。歩く方式だと `.lighthouseci/` の Lighthouse レポートのような配信されない生成物まで拾い、**§2 の手順どおり `npm run test:lighthouse` を流しただけで次の `check:csp` が赤くなる**（それを除外リストで避けると、生成物が増えるたびに育つ手書きの写しがまた 1 つ生まれる）。

実装上の注意（いずれも実際に取り違えた／レビューで指摘された点）:

- **HTML コメントは script より先に消費する**（走査を `<!--...-->|<script...>...</script>` の 1 本の交替で書く）。`<!-- 以下が唯一の <script> ブロック -->` のような**注釈を 1 行足すだけ**でコメント途中から script として拾い、本来正しいページに「不一致」と報告したうえ、**貼り替えると CI が緑になるのにブラウザは拒否し続ける**値を出す。`resume.html` は実際に `NO <script>` と書いたコメントを持っている。
- **開始タグの属性は引用符を考慮して切る。** `[^>]*` だと `<script onerror="if(a>b)f()">` を最初の `>` で切ってしまい、上とまったく同じ「貼り替えると CI だけ緑」の値を出す。
...
- **meta の属性は並びや引用符を決め打ちせず、script と同じ属性パーサで解釈する。** `http-equiv="…"` の直後に `content="…"` が来る形だけを見ていると、**属性を入れ替えて書いた 2 枚目の CSP が丸ごと見えず**複数 meta の門番を素通りする（逆に、`content` 先行で書いた正しい CSP を「見つかりません」と誤検知もする）。
- **CSP のキーワード（`'none'` 等）も小文字化して比べる。** 名前だけ小文字化して値を素の文字列で比べると、`script-src 'None'` という正当な最小権限のページを「緩い」と誤検知して CI が赤くなる。
- **規則 A の案内も規則 B と同じ分岐を使う。** `default-src` へフォールバックしているページに「そこへ sha256 を足せ」と案内すると、画像・スタイル・フォントの許可まで広がる。2 つの規則が同じページ形状に矛盾した指示を出さないよう、判断は 1 か所（`fallsBackToDefaultSrc`）に置く。
...
**この検査が見ていないもの**: `'unsafe-eval'` の追加や許可ホストの追加など、`script-src` の**他の緩みは検査しない**（見るのは sha256 とインライン script の対応だけ）。成功時の出力もその範囲だけを名乗る。CSP 全体の妥当性はレビューで見る。
...
- スタック形態は「静的サイト」: GitHub Pages の**公開デモ URL 必須**（`https://izumacha.github.io/profile-portfolio/`）。README 冒頭のデモブロックにバッジ/リンクで記載する。
- 撮影対象スクリーンショット（`docs/screenshots/` 配下）:
  1. `index-hero-dark.png` — メインページのファーストビュー（ダークテーマ）
  2. `index-sections-dark.png` — スキル/タイムライン等の主要セクション
  3. `resume-light.png` — 履歴書ページ（ライトテーマ）
  - 任意の追加候補: `index-mobile-768.png` — 768px モバイル表示
- デモ GIF: ナビゲーションからのセクション遷移＋スクロールアニメーション 1 本（10MB 以下）。
- 撮影は既存の Playwright 環境（§2 の `test:e2e`）を再利用した自動撮影スクリプトを推奨。ポートフォリオは公開情報のみだが、スクショに写るメールアドレス等の連絡先は本人が公開を意図したものに限る。
