...
- **規則 A**: `script-src` の sha256 トークンの集合と、実行対象インライン script のハッシュの集合が**過不足なく一致する**こと。1 対 1 で突き合わせるので「ハッシュが古い」「script を足したのにトークンを足していない」「script を消したのにトークンが残っている」がすべてこの 1 つで落ちる。**不一致のときは追加・削除すべきトークンをそのまま貼り替えられる形で出す。**
...
実装上の注意（いずれも実際に取り違えた／取り違えうる点）:

- **HTML コメントは script より先に消費する**（走査を `<!--…-->|<script…>…</script>` の 1 本の交替で書く）。`<!-- 以下が唯一の <script> ブロック -->` のような**注釈を 1 行足すだけ**でコメント途中から script として拾い、正しいページに「不一致」と報告したうえ、**貼り替えると CI が緑になるのにブラウザは拒否し続ける**値を出す。`resume.html` は実際に `NO <script>` と書いたコメントを持っている。
...
- **ハッシュは改行を LF へ正規化してから計算する**。配信されるのはコミットされた blob（`.gitattributes` で LF 固定）なので、Windows の CRLF チェックアウトでずれないようにする（§10）。
