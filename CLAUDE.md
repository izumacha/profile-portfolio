...
導出の健全性を見るガード（`検査対象の導出が壊れていない`）も同じファイルにあり、**ページを 1 枚も導出できない** / `.htmlvalidateignore` に導出が解釈できない書き方（`!` 否定など）がある / **`.gitignore` の隠しでないディレクトリが `.htmlvalidateignore` に載っていない** の 3 つで落ちる。3 つ目は、html-validate が `.gitignore` を読まないため生成物ディレクトリの一覧が 2 か所に必要になることへの手当て（**生成物ディレクトリを足すときは両方に書く**。片方だけだと、生成物が存在しない CI は緑のまま手元だけが赤くなる）。
